...
class MixedCargoContainerModelTest(unittest.TestCase):
	...
	def test_set_weight_capacity_raises_an_exception_if_the_requested_capacity_is_less_than_or_equal_to_zero(self):
		bag = MixedCargoContainer()
		with self.assertRaises(CargoContainerException) as error_context:
			bag.set_weight_capacity(0)
		self.assertEqual("Weight capacity must be greater than 0", error_context.exception.message)

	def test_set_weight_capacity_raises_an_exception_if_the_requested_capacity_is_less_than_or_equal_to_the_current_weight_in_the_container(self):
		bag = MixedCargoContainer()
		...
		self.assertEqual("Weight capacity must be greater than or equal to the current weight", error_context.exception.message)

	def test_get_current_weight_computes_the_total_weight_of_all_resource_items_in_the_container(self):
		bag = MixedCargoContainer()
	...
	def test_remaining_capacity_computes_the_number_of_resource_type_that_can_still_be_held_in_the_container(self):
		bag = MixedCargoContainer()
		bag.set_weight_capacity(100)
		slot = bag.get_slot(Fish)
		slot['load'] = 3
	...
	def test_current_load_returns_the_current_number_of_resource_type_held_in_the_container(self):
		bag = MixedCargoContainer()
		bag.set_weight_capacity(100)
		slot = bag.get_slot(Fish)
		slot['load'] = 3
	...
	def test_load_cargo_increases_load_of_slot_if_bag_can_hold_the_resource_and_returns_excess_that_could_not_be_loaded(self):
		bag = MixedCargoContainer()
	...
	def test_load_cargo_returns_entire_load_if_slot_is_already_at_capacity(self):
		bag = MixedCargoContainer()
		bag.set_weight_capacity(Fish.weight * 3)
	...
	def test_unload_cargo_decreases_load_of_container_and_returns_all_available_cargo_up_to_the_requested_quantity(self):
		bag = MixedCargoContainer()
		bag.set_weight_capacity(Fish.weight * 10)
		bag.get_slot(Fish)['load'] = 3
	...
	def test_unload_cargo_returns_nothing_if_bag_has_no_resources_of_requested_type(self):
		bag = MixedCargoContainer()
		bag.set_weight_capacity(Fish.weight * 10)
		load = bag.unload_cargo(Fish, 7)
		self.assertEqual(bag.current_load(Fish), 0)
...
class SlottedCargoContainerModelTest(unittest.TestCase):
	...
	def test_load_cargo_increases_load_of_slot_if_tray_can_hold_the_resource(self):
		tray = SlottedCargoContainer()
		tray.add_resource_slot(Fish, 1)
	...
	def test_load_cargo_increases_load_of_slot_if_tray_can_hold_the_resource_and_returns_excess_that_could_not_be_loaded(self):
		tray = SlottedCargoContainer()
	...
	def test_load_cargo_raises_exception_if_tray_cannot_hold_the_resource(self):
		tray = SlottedCargoContainer()
		with self.assertRaises(CargoContainerException) as error_context:
			tray.load_cargo(Fish, 1)
		self.assertIn("A cargo slot of that type has not been added", error_context.exception.message)

	def test_unload_cargo_decreases_load_of_slot_if_tray_can_hold_the_resource_and_all_available_cargo_up_to_the_requested_quantity(self):
		tray = SlottedCargoContainer()
		tray.add_resource_slot(Fish, 3)
		tray.get_slot(Fish)['load'] = 3
	...
	def test_unload_cargo_raises_exception_if_tray_cannot_hold_the_resource(self):
		tray = SlottedCargoContainer()
		with self.assertRaises(CargoContainerException) as error_context:
