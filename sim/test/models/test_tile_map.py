import unittest
from sim.geometry import Point, Size
...
class TileMapModelTest(unittest.TestCase):
	...
	def test_initialization_raises_an_exception_if_the_tile_size_has_an_area_of_zero(self):  # noqa
		with self.assertRaises(TileMapException) as error_context:
			TileMap(TileGrid(Size(10, 10)), tile_sz=Size(0, 0))
		self.assertIn(
			"The tile size must have positive area",
	...
	def test_place_unit_on_grid_puts_a_unit_onto_the_map_in_the_specified_grid_cell(self):  # noqa
		tmap = TileMap(TileGrid(Size(10, 10)), tile_sz=Size(20, 20))
		serf = Peasant()
	...
	def test_map_coords_to_grid_coords_converts_tile_map_coordinates_to_tile_grid_coordinates(self):  # noqa
		tmap = TileMap(TileGrid(Size(10, 10)))
		self.assertEqual(
			Point(10, 15),
			tmap.map_coords_to_grid_coords(Point(200, 300)),
			)
		self.assertEqual(
			Point(10, 15),
			tmap.map_coords_to_grid_coords(Point(219, 319)),
			)
		self.assertEqual(
			Point(11, 16),
			tmap.map_coords_to_grid_coords(Point(220, 320)),
			)

	def test_grid_coords_to_map_coords_converts_tile_grid_coordinates_to_tile_map_coordinates(self):  # noqa
		tmap = TileMap(TileGrid(Size(10, 10)))
		self.assertEqual(
			Point(200, 300),
			tmap.grid_coords_to_map_coords(Point(10, 15)),
			)
		self.assertEqual(
			Point(220, 320),
			tmap.grid_coords_to_map_coords(Point(11, 16)),
			)
