...
class MoveTowardTest(unittest.TestCase):
	...
	def test__execute_does_not_moves_the_unit_toward_the_target_point_if_the_tile_containing_that_point_is_non_traversable_and_sets_is_obstructed_to_true(self):
		dummy_tile = MagicMock()
		dummy_tile.terrain.movement_factor = 0.5
		dummy_tile.terrain_improvement.movement_factor = 0.25
		...
		movement = MoveToward(Point(3, 4))
		movement._execute(dummy_unit, 2.0)
		dummy_unit.move.assert_not_called(Vector(2, 2).u*min(3*0.5*0.25*7*2.0,Vector(2, 2).M))
		self.assertTrue(movement.is_obstructed)

	def test_is_complete_returns_true_if_unit_is_at_or_very_near_the_target_point(self):
		dummy_unit = MagicMock()
		dummy_unit.pt = Point(3 + EPS/2, 4)
